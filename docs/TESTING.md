...
- Diagnostics must cover consent, quotas, multipart bounds, token binding, default-on pre-verification and its audited administrator toggle, stored-without-verification behavior when disabled, explicit-download priority, bounded concurrent full-object byte/hash verification, administrator deletion, fair retained-capacity eviction, provider-failure re-accounting, deletion retries, and cleanup expiry.
