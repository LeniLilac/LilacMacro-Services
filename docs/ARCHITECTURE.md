...
The official client uploads diagnostics only through the default-off automatic-report choice. Every archive uses one 3 GiB maximum and, after exact size and SHA-256 verification, is accepted for 72 hours. There is no manual-upload API contract, large-file grant, Pending acceptance workflow, or retention extension.
