...
Administrators may create random, explicitly scoped API keys for programmatic access. The database stores only a SHA-256 hash of each 256-bit random secret plus a short display suffix, selected scopes, creator, expiry, revocation, and bounded usage metadata. A key expires after 7, 30, or 90 days. Closed scopes independently authorize control reads and administrator-owned changes, diagnostic metadata, on-demand verification/download, deletion, bounded telemetry aggregates, audit, and key management through `/v1/admin-data`. Control writes still use the same closed command envelope and cannot invoke system-owned release or game-observation commands. Diagnostic writes still use the existing lifecycle service. Every such action records the API key UUID as its actor in the immutable control or diagnostic audit. A key with `keys:manage` may create only child keys whose scopes are a subset of its own and whose expiry is no later than its own, or revoke an existing key. No key can reach raw telemetry, object-storage credentials, signing keys, or internal service routes. The catalog lists only methods authorized for that key. Browser key creation and revocation retain normal session and CSRF proof. Bearer-key endpoints are separately rate limited and always return `no-store`.

Telemetry accepts at most 64 events and 64 KiB per request from the exact public endpoint. Zod and PostgreSQL both enforce the same discriminated event-kind, supported privacy-notice-version range, enum, numeric, timestamp, known-vendor model-name, display-dimension, and UI-scale bounds, including the closed OCR-setup and local-instance failure taxonomies emitted by current clients. The supported notice range keeps earlier valid clients ingesting when the current notice changes; unknown future notices fail closed. The API immediately HMAC-pseudonymizes the random client installation UUID and normalized source network with separate telemetry-specific monthly domains; the database stores neither source value, and neither diagnostic pseudonym can be joined to them. The network pseudonym is used only for the daily abuse budget. Fixed columns prevent arbitrary JSON/log retention. The worker deletes rows and capacity records after 90 days, while authenticated administrators see at most 250 grouped counts, estimated rotating installations, average timings by normalized CPU/GPU model, display/UI-scale calibration pairs, quantity totals, and the latest event time through a security-definer function. Public clients remain unauthenticated, so aggregates are untrusted and may be forged or denied despite bounded quotas; they are advisory only and never automatically alter application or control policy.
...
The official client uploads diagnostics only through its separate Deep Debug error-upload choice. Every archive uses one 3 GiB maximum and is retained for at most 72 hours from upload. Completed archives remain stored but unavailable to download until an administrator requests one; only that request queues exact size and SHA-256 verification. There is no manual-upload API contract, light-report fallback, large-file grant, administrator acceptance workflow, or retention extension.

Administrators may explicitly delete accepted or stalled archives after review. The operation first claims lifecycle ownership transactionally, records the administrator in immutable audit, removes or aborts the provider object, and reports `Deleted` only after provider confirmation. Failures enter the same bounded deletion-retry lifecycle as retention cleanup. The authenticated console can filter archive metadata by a user-supplied installation UUID without persisting or returning that raw value: a no-store POST body is converted to the current and previous monthly diagnostic HMAC pseudonyms before the bounded query. The global retained allocation is exactly 1,000 GB, or 720 decimal TB-hours over a 30-day month, below the project's 730 TB-hour allowance. When an incoming archive would exceed that allocation, admission transactionally queues the oldest evictable archive from the installation with the most retained archives, recalculates counts, and repeats only until the incoming archive fits. Equal counts are resolved by the oldest archive globally. An archive actively uploading, completing, verifying, or deleting cannot be displaced; admission fails closed if eligible archives cannot release enough capacity. A capacity eviction releases the logical allocation immediately and is prioritized for provider deletion. A failed provider deletion restores the archive to retained-capacity accounting before retry. Legacy grant tables and Pending metadata remain only for rollback-safe cleanup and have no issuance or acceptance path.
