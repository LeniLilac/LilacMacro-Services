...
Administrator-supplied diagnostic installation UUID filters are also untrusted. They require an authenticated session plus CSRF proof, accept one exact UUID in a no-store request body, and are immediately reduced to the two monthly diagnostic pseudonyms that can overlap the 72-hour retention window. The raw UUID is not returned, added to archive metadata, or placed in request URLs.
