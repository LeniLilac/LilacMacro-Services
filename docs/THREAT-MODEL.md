...
Administrator-supplied diagnostic filters are also untrusted. Browser searches require an authenticated session plus CSRF proof; API searches require the exact `diagnostics:read` scope. A raw installation UUID is accepted only in a no-store request body and immediately reduced to the two monthly diagnostic pseudonyms that can overlap the 72-hour retention window. The raw UUID is not returned, added to archive metadata, or placed in request URLs. Version, operating-system, time, size, and limit fields use closed bounded schemas and parameterized repository queries.
...
| Multipart confusion             | scoped token; exact part size/checksum grants; assembled-size check; bounded concurrent full-object size and SHA-256 verification before download                                                                                  |
